...
var Selection =
  ...
  function () {
    ...
    _proto._addInitialEventListener = function _addInitialEventListener() {
      ...
      var mouseDownListener = addEventListener('click', function (e) {
...
function eventSegments(event, range$$1, accessors) {
  ...
  var eventStartSplited = eventStart.split(" ");
  var eventEndSplited = eventEnd.split(" ");
  var getStartEventDay = eventStartSplited[2];
  var getEndEventDay = eventEndSplited[2];
  var span = getEndEventDay - getStartEventDay;

  console.log(getStartEventDay + " " + getEndEventDay);

  if (getStartEventDay === getEndEventDay) {
    span = 1;
  } else {
    span = span + 1;
  }
  ...
  function (_React$Component) {
    ...
    function MonthView() {
      ...
      _this.handleSelectSlot = function (range$$1, slotInfo) {
        ...
        var getEventDate = Moment(range$$1[0]);
        var getActualDate = Moment();
        var isAfter = Moment(getEventDate).isAfter(getActualDate);
        var getAllDates = _this.props.events;
        var alreadyExist = true;

        getAllDates.forEach(function (item) {
          var date = item.start._i.toString();
          var selected = range$$1[0].toString();
          if (date == selected) {
            alreadyExist = false;
          }
        })

        if (isAfter && alreadyExist) {
