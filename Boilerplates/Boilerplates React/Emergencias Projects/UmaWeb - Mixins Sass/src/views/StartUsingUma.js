...
const StartUsingUma = (props) => {
    ...
    const sendPhoneCall = (inputPhone) => {
        ...
        Axios.post(url + '/send_hsm', dataHsm, { 'Content-type': 'application/json' } )
            ...
            .catch(function (err) {
                console.log("[ONLINE-DOCTOR-DASHBOARD] /send_hsm Ocurrió un error", err);
                setLoading(false);
                ...
                { successSending ? 
                    <div>                        
                        <div className="closeButtonContainer">
                            <FontAwesomeIcon icon={faTimes} onClick={() => props.closeModal(false)}/>
                        </div>
                        ...
                        : 
                            <div className="sendDescription">
                                ...
                                <div className="followInstructions">Siga las instrucciones para continuar.</div> 
                            </div>
                        }
                    </div>
                : 
                    <div>
                        ...
                        <img src={logo} className="login-logo" alt="UMA"/>
                        <h5>Consulta médica online y mucho más!</h5>
                        <div>
                            <label>Regístrate con tu whatsapp</label><br /><br />
                            <div className="inputNumberContainer">
                                <PhoneInput country={'ar'}
                                    placeholder={'Escribe tu número'}
                                    value={ phone }
                                    onChange={ (value) => setPhone(value) } />
                            </div>
                            <br />
                            <button className="btn btn-active buttonModal" disabled={!isChecked} onClick={() => sendPhoneCall(phone)}>Enviar</button>
                            <div className="sendDescription">
                            ...
                            <div className="acceptTermsAndConditions">
                                ...
                                <Link to="/terminos" target="_blank" className="termsText">Acepto los términos y condiciones y la política de privacidad</Link>
                            </div>
                        </div>
                    </div>
                } 
...
export default withRouter(StartUsingUma);
